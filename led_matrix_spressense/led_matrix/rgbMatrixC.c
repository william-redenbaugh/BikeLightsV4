...
void DispShowChar(char chr,byte R,byte G,byte B,char bias=0,char direction=3){
  char i,j;
  ...
  for(i = 0;i< 8;i++)
  {
    chrtemp[j] = pgm_read_byte(&(font8_8[Char][i]));    //moving the array is equivalent to moving characters
    j++;
  }
...
void flow_text(char str[],byte R,byte G,byte B,char direction=3,int delayms=100){
  int len = strlen(str);
...
/*  j = up_down;
  for(i = 0;i< 8;i++)
  {
    chrtemp[j] = hex[i];    //moving the array is equivalent to moving characters
	j++;
  }  */
...
//  Serial.println(cmd);
  ...
  int ex = ioctl(i2c_fd, I2C_TRANSFER, (unsigned long)(uintptr_t)&i2c_transfer);
