...
void char2rgb_line(char charline, char row, char column, uint8_t R, uint8_t G, uint8_t B)
{
  if (charline & 0x80)
  {
...
void disp_show_char(char chr, uint8_t R, uint8_t G, uint8_t B, char bias, char direction)
{
  char i, j;
...
void flowchar(char chr, uint8_t R, uint8_t G, uint8_t B)
{
  for (int i = 5; i > -8; i--)
  {
    disp_show_char(chr, R, G, B, i, 0);
    delay(50);
  }
}

void show_text(char str[], uint8_t R, uint8_t G, uint8_t B)
{
  int len = strlen(str);
  for (int i = 0; i < len; i++)
  {
...
void show_hex(uint8_t hex[], uint8_t R, uint8_t G, uint8_t B, char bias)
{
  char i, j, temp;
  unsigned char chrtemp[24] = {0};
...
void disp_show_pic(uint8_t Index, uint8_t R, uint8_t G, uint8_t B)
{
  int data_val = 0;
  for (int i = 0; i < 64; i++)
  {
...
void check_err_i2c(int result)
{
  ...
  case ENOTTY:
    ...
    break;
  default:
    printf("Unkown I2C error has been found");
    break;
  }
...
void write_cmd(uint8_t reg, uint8_t cmd)
{
  ...
  i2c_transfer.msgc = 1;
  int transfer_result = ioctl(i2c_fd, I2CIOC_TRANSFER, (unsigned long)(uintptr_t)&i2c_transfer);
...
void write_array_data(uint8_t startaddr, uint8_t data[64], uint8_t data_len)
{
...
void write_n_data(uint8_t startaddr, uint8_t data, uint8_t data_len)
{
  for (int i = 0; i < data_len; i++)
  {
...
void image(uint8_t image[64][3])
{
  ...
  for (int i = 0; i < 15; i++)
  {
    ...
    int result = ioctl(i2c_fd, I2CIOC_TRANSFER, (unsigned long)(uintptr_t)&i2c_transfer);
...
void RGBMatrixInit()
{
  ...
  write_n_data(0x00, 0X00, 0XB3);
  write_cmd(CONFIGURE_CMD_PAGE, FRAME2_PAGE);
  write_n_data(0x00, 0X00, 0XB3);
  write_cmd(CONFIGURE_CMD_PAGE, LED_VAF_PAGE);
  write_array_data(0X00, tab_led_type3Vaf, 0X40);
  ...
  write_n_data(0X00, 0XFF, 0X10); // Clear LED CTL Registers (Frame1Page)
  write_n_data(0x20, 0x00, 0X80);
  write_cmd(CONFIGURE_CMD_PAGE, FRAME2_PAGE);
  write_n_data(0X00, 0XFF, 0X10); // Clear LED CTL Registers (Frame1Page)
  write_n_data(0x20, 0x00, 0X80);
  // set_matrix_full(255);
